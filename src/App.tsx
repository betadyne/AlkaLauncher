...
function App() {
  ...
  const [page, setPage] = createSignal<
    "library" | "detail" | "detail-info" | "detail-chars"
  >("library");
  ...
  const saveToken = async () => {
    ...
    await invoke("save_vndb_token", { token: tokenInput() });
    await loadSettings();
    setTokenInput("");
  };
  const clearToken = async () => {
    await invoke("clear_vndb_token");
    setAuthUser(null);
    await loadSettings();
  };
  const toggleBlurNsfw = async () => {
    await invoke("set_blur_nsfw", { blur: !settings().blur_nsfw });
    await loadSettings();
  };
  const addGame = async () => {
    ...
    if (sel) {
      setLoading(true);
      const g = await invoke<Game>("add_local_game", { path: sel });
      await loadGames();
      setLoading(false);
      setSearchModal(g);
  ...
  const hideGame = async (id: string, hidden: boolean) => {
    await invoke("set_game_hidden", { id, hidden });
    await loadGames();
  };
  ...
  const linkVndb = async (vndb: VndbSearchResult) => {
    const g = searchModal();
    if (!g) return;
  ...
  const openDetail = async (game: Game, forceRefresh = false) => {
    ...
    const [d, c] = await Promise.all([
      invoke<VndbVnDetail>("fetch_vndb_detail", {
        vndbId: game.vndb_id,
        forceRefresh,
      }),
      invoke<VndbCharacter[]>("fetch_vndb_characters", {
        vndbId: game.vndb_id,
        forceRefresh,
      }),
    ]);
    ...
    if (settings().vndb_token) {
      try {
        setUserVn(
          await invoke<VndbUserListItem | null>("vndb_get_user_vn", {
            vndbId: game.vndb_id,
          }),
        );
      } catch {
  ...
  const setStatus = async (labelId: number) => {
    const g = currentGame();
    if (!g?.vndb_id) return;
    await invoke("vndb_set_status", { vndbId: g.vndb_id, labelId });
    setUserVn(
      await invoke<VndbUserListItem | null>("vndb_get_user_vn", {
        vndbId: g.vndb_id,
      }),
  ...
  const setVote = async (vote: number) => {
    const g = currentGame();
    if (!g?.vndb_id) return;
    await invoke("vndb_set_vote", { vndbId: g.vndb_id, vote });
    setUserVn(
      await invoke<VndbUserListItem | null>("vndb_get_user_vn", {
        vndbId: g.vndb_id,
      }),
  ...
  const shouldBlur = (img: VndbImage | null): boolean =>
    !!(settings().blur_nsfw && img && (img.sexual >= 1 || img.violence >= 1));
  ...
  listen<{ game_id: string; play_minutes: number }>(
    "game-exited",
    async (event) => {
            ...
            <div class="p-4 space-y-4">
              ...
              <div class="flex items-center justify-between">
                ...
                <button
                  ...
                  class={`w-10 h-5 rounded-full transition-colors ${settings().blur_nsfw ? "bg-green-400" : "bg-slate-600"}`}
                >
                  <div
                    class={`w-4 h-4 bg-white rounded-full transition-transform ${settings().blur_nsfw ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
          ...
          <div class="bg-slate-800 rounded-lg w-full max-w-lg max-h-[70vh] overflow-hidden">
            ...
            <div class="p-3">
              ...
              <div class="overflow-y-auto max-h-80 space-y-1">
                <Show when={searching()}>
                  <p class="text-gray-400 text-center py-3 text-sm">
                    Searching...
                  </p>
                </Show>
                      ...
                      <div class="w-10 h-14 bg-slate-600 rounded overflow-hidden flex-shrink-0">
                        <Show when={r.image?.url}>
                          <img
                            src={r.image!.url}
                            alt={r.title}
                            class={`w-full h-full object-cover ${shouldBlur(r.image) ? "blur-lg" : ""}`}
                          />
                        </Show>
                      </div>
